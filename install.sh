...
download() {
    ...
    core)
        is_core_ver="v1.9.7"
        link="github.com/${is_core_repo}/releases/download/${is_core_ver}/${is_core}-${is_core_ver:1}-linux-${is_arch}.tar.gz"
        [[ -n $px ]] && link="${px}${link}"
        name=$is_core_name
        tmpfile=$tmpcore
        is_ok=$is_core_ok
        ;;
    sh)
        link="github.com/${is_sh_repo}/releases/latest/download/code.tar.gz"
        [[ -n $px ]] && link="${px}${link}"
        ...
        ;;
    jq)
        link="github.com/jqlang/jq/releases/download/jq-1.7.1/jq-linux-$is_arch"
        [[ -n $px ]] && link="${px}${link}"
        name="jq"
        tmpfile=$tmpjq
        is_ok=$is_jq_ok
...
# get server ip
get_ip() {
    export "$(_wget -4 -qO- https://www.visa.cn/cdn-cgi/trace | grep ip=)" &>/dev/null
    [[ -z $ip ]] && export "$(_wget -6 -qO- https://www.visa.cn/cdn-cgi/trace | grep ip=)" &>/dev/null
}
