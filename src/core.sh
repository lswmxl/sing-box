...
get_ip() {
    ...
    [[ ! $ip ]] && export "$(_wget -6 -qO- https://www.visa.cn/cdn-cgi/trace | grep ip=)" &>/dev/null
    [[ ! $ip ]] && {
        err "获取服务器 IP 失败.."
    }
}
...
get() {
    ...
    protocol)
        ...
        *)
        ...
        ;;
    host-test) # 检查DNS记录，TLS协议需要
        [[ $is_no_auto_tls || $is_gen || $is_dont_test_host ]] && return
        get_ip
        get ping
        if [[ ! $(grep $ip <<<$is_host_dns) ]]; then
            msg "\n请将 ($(_red_bg $host)) 解析到 ($(_red_bg $ip))"
            msg "\n如果使用 Cloudflare, 在 DNS 那里; 请关闭小黄云"
            ask string y "我已经确定解析 [y]:" 
            get ping
            if [[ ! $(grep $ip <<<$is_host_dns) ]]; then
                _cyan "\n测试结果: $is_host_dns"
                ask string confirm_skip "域名 ($host) 没有解析到 ($ip)，是否跳过检查并继续？(输入 y 跳过，按回车继续检查):"
                if [[ "$confirm_skip" == "y" ]]; then
                    msg "用户选择跳过 DNS 检查，继续执行下一个流程。"
                    return
                fi
                err "域名 ($host) 没有解析到 ($ip) 脚本停止"
            fi
        fi
        ;;
